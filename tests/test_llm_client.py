...
def test_answer_question_includes_history_for_reference_resolution() -> None:
    with patch.dict("os.environ", {"GOOGLE_API_KEY": "test-key"}):
        with patch("llm_client.genai") as genai:
            client_class = genai.Client
            client_class.return_value.models.generate_content.return_value.text = "answer"
            ...
            answer_question(
                ...
                ["retrieved context"],
                history=[{"question": "What is this?", "answer": "A report."}],
            )

    prompt = client_class.return_value.models.generate_content.call_args.kwargs[
        "contents"
    ]
    assert "Previous conversation:" in prompt
    assert "User: What is this?\nAssistant: A report." in prompt
    assert "only to resolve references" in prompt
